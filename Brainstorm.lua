-- Brainstorm OVERHAULED Mod for Balatro
...
local UI_MODULE = "brainstorm_overhauled_ui"
...
local function init_ffi()
  if not ffi_loaded then
    local success = pcall(
      ...
      void hex_set_cuda_enabled(bool enabled);
...
function Brainstorm.auto_reroll()
  ...
  local hex = load_native()
  if not hex then
    return false, "Auto-reroll stopped (Hex.dll missing or incompatible)"
  end
  ...
  if hex.cuda_enabled ~= cuda_enabled then
    local cuda_success, cuda_error = pcall(hex.set_cuda_enabled, cuda_enabled)
    ...
    hex.cuda_enabled = cuda_enabled
  end

  local call_success, result = pcall(
    hex.brainstorm_search,
    seed_start,
  ...
  local free_success, free_err = pcall(hex.free_result, result)
