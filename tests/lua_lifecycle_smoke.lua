...
package.preload.ffi = function()
  return {
    ...
    load = function(path)
      assert(path == repo .. "/Brainstorm/Hex.dll")
      ...
      local handle = {
        ...
        hex_set_cuda_enabled = function(enabled)
      ...
      if omit_cuda_export then
        handle.hex_set_cuda_enabled = nil
      end
...
package.preload.brainstorm_overhauled_ui = function()
...
assert(
  not Brainstorm.normalize_config({ ar_prefs = { use_cuda = "legacy" } }).ar_prefs.use_cuda
)
assert(
...
assert(
  missing_result == false
    and missing_error
      == "Auto-reroll stopped (Hex.dll missing or incompatible)"
)
...
assert(
  ffi_declarations:find("void hex_set_cuda_enabled(bool enabled);", 1, true)
)
