-- Brainstorm OVERHAULED UI Module
-- Brainstorm OVERHAULED is KRVH's full rewrite of OceanRamen's original Brainstorm.
-- Hex is KRVH's Rust rewrite of the core seed-search engine from
-- MathIsFun0's original Immolate, source-verified and extended for Brainstorm.
...
-- Balatro uses "tag_skip" for Speed Tag.
local tag_list = {
  ["None"] = "",
...
local function refresh_brainstorm_tab()
  ...
  local tab_button =
    G.OVERLAY_MENU:get_UIE_by_ID("tab_but_Brainstorm OVERHAULED")
  if tab_button then
...
function Brainstorm.build_settings_tab()
  return {
      ...
      return {
        ...
        nodes = {
          ...
          },
          {
            ...
            nodes = {
              create_toggle({
                label = "Enable Brainstorm OVERHAULED",
                scale = 0.8,
                ref_table = config,
                ref_value = "enable",
              ...
              create_toggle({
                label = "AR: Use CUDA (Experimental)",
                scale = 0.8,
                ...
                nodes = {
                  {
                    n = G.UIT.T,
                    config = {
                      text = "Brainstorm OVERHAULED: KRVH's full rewrite",
                      scale = 0.28,
                      colour = G.C.UI.TEXT_LIGHT,
                    },
                  },
                ...
                nodes = {
                  {
                    n = G.UIT.T,
                    config = {
                      text = "of OceanRamen's original Brainstorm.",
                      scale = 0.28,
                      colour = G.C.UI.TEXT_LIGHT,
                    },
                  },
                ...
                nodes = {
                  {
                    n = G.UIT.T,
                    config = {
                      text = "Hex: KRVH's Rust rewrite of the core seed-search",
                      scale = 0.28,
                      colour = G.C.UI.TEXT_LIGHT,
                    },
                  },
                ...
                nodes = {
                  {
                    n = G.UIT.T,
                    config = {
                      text = "engine from MathIsFun0's original Immolate.",
                      scale = 0.28,
                      colour = G.C.UI.TEXT_LIGHT,
                    },
                  },
    ...
    tab_definition_function_args = "Brainstorm OVERHAULED",
  }
end
...
local function has_brainstorm_tab(tabs)
  for _, tab in ipairs(tabs) do
    if tab and tab.label == "Brainstorm OVERHAULED" then
      return true
    end
  end
  return false
