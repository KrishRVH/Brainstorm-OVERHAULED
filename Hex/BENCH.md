...
benchmark smoke tests. `mise run bench-compare` measures performance against the
...
CUDA acceleration is experimental. These gates are intentionally stricter than
ordinary benchmark reporting so the shipped CPU fallback remains authoritative.
...
`Brainstorm.auto_reroll()` inside Balatro.
...
Build the Rust DLL used by Brainstorm OVERHAULED:
...
```bash
BENCH_BASELINE_DLL=/path/to/frozen/Hex.dll \
...
`bench-compare` uses
`Hex/Original-Immolate-OceanRamenandMathIsFun0.dll` by default. Override it
with `ORIGINAL_DLL=/path/to/Original-Immolate.dll` or
`BENCH_ORIGINAL_DLL=/path/to/Original-Immolate.dll`.
...
- CUDA Toolkit 12.4 with `nvcc` and GCC 12 for the GPU-enabled build. Set
  `BRAINSTORM_SKIP_CUDA_BUILD=1` only for an intentional CPU-only build.
  The production fat binary contains native kernels for compute capabilities
  5.0, 5.2, 6.0, 6.1, 7.0, 7.5, 8.0, 8.6, 8.9, and 9.0 plus an 8.9 PTX
  fallback. Advanced builds can override `BRAINSTORM_CUDA_ARCHES`,
  `BRAINSTORM_CUDA_PTX_ARCH`, `CUDAHOSTCXX`, or `NVCC`. An incompatible module
  falls back to Rust CPU and therefore fails strict `cuda-long` backend
  attestation. Initialization and runtime failures remain latched until the
  harness process restarts.
...
- `ORIGINAL_DLL=Hex/Original-Immolate-OceanRamenandMathIsFun0.dll`
...
fixture is strictly comparable: both implementations return the same non-empty
...
`hex_last_search_used_cuda` export; it is not inferred from timing.
...
OVERHAULED behavior. Set `BENCH_FAIL_ON_MISMATCH=1` only when intentionally
...
- `cuda-long-*`: supported long-window fixtures for native-Windows CPU/CUDA
  parity and throughput measurements, including full-budget misses for both
  the generic and specialized Spectral Soul kernels.
...
100M seed cap. A legacy hit beyond the requested current search budget is also
skipped because it is outside the measured window. The harness skips an empty
legacy result because the old ABI uses the same empty string for a successful
empty seed and for no hit, so the work is ambiguous. Each measured
current/legacy case gets one untimed probe before its configured warmups, and
every measured repeat must return exactly the probe's raw seed and scan count.
...
```bash
cargo run --manifest-path Hex/Cargo.toml --release --bin brainstorm_bench -- \
...
When adding a benchmark fixture, update `Hex/src/bench_cases.rs`. Both
`Hex/src/bin/hex_dll_harness.rs` and `Hex/src/bin/brainstorm_bench.rs` read
