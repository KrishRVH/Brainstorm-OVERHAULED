...
mod tests {
    ...
    use crate::engine::search::resolve_seed_budget;
    ...
    use crate::item::{
        COMMON_JOKERS, ITEM_COUNT, Item, LEGENDARY_JOKERS, RARE_JOKERS, UNCOMMON_JOKERS,
        item_to_string, string_to_item,
    };
    ...
    fn seed_budget_resolution_is_explicit() {
        assert_eq!(resolve_seed_budget(0), 0);
        assert_eq!(resolve_seed_budget(-1), 0);
        ...
        let no_filters = FilterConfig::default();
        assert_eq!(brainstorm_search_core("", &no_filters, 0, 0), None);
        assert_eq!(brainstorm_search_core("", &no_filters, -1, 0), None);
        assert_eq!(
            brainstorm_search_core("", &no_filters, 1, 0),
            Some(String::new())
        );
    }

    #[test]
    fn no_filter_shapes_respect_budget_and_preserve_canonical_start() {
        ...
        for cfg in [FilterConfig::default(), forced_buffoon] {
            ...
            for (start, canonical) in [
                ...
                for budget in [-1, 0, 1, SEED_SPACE + 1] {
                    let expected = (budget > 0).then_some(canonical);
    ...
    #[test]
    fn source_oracle_target_seeds_cover_every_search_filter() {
        let cases = [
    ...
    #[test]
    fn current_joker_pools_match_source_catalog() {
