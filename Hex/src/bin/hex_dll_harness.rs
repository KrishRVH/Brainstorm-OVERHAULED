...
#[cfg(not(windows))]
fn main() {
    eprintln!("hex_dll_harness must be built for Windows and run under Windows or Wine");
    std::process::exit(2);
}
...
#[cfg(any(windows, test))]
fn classify_legacy_probe(
    start: &str,
    result: Option<&str>,
    budget: i64,
) -> Result<LegacyProbe, String> {
    ...
    Ok(if scanned > budget {
        LegacyProbe::OutOfBudget
    } else {
...
fn attest_backend(
    ...
    let used_cuda = used_cuda.ok_or("DLL does not export hex_last_search_used_cuda")?;
...
mod tests {
    ...
    fn legacy_probe_classifies_hits_and_ambiguous_empty_results() {
        ...
        assert_eq!(
            classify_legacy_probe("", Some("A"), 11),
            Ok(LegacyProbe::Hit { scanned: 11 })
        );
        assert_eq!(
            classify_legacy_probe("", Some("A"), 10),
            Ok(LegacyProbe::OutOfBudget)
        );
        assert_eq!(
            classify_legacy_probe("", Some(""), 10),
            Ok(LegacyProbe::EmptyResult)
        );
        assert!(classify_legacy_probe("", None, 10).is_err());
    }
...
mod windows_harness {
    ...
    use hex::{CompiledFilter, FilterConfig, SEED_SPACE, Seed};
    ...
    impl Dll {
        ...
        fn load_current(path: &str) -> Result<Self, String> {
            ...
            let cuda_ptr = unsafe { GetProcAddress(handle, c"hex_set_cuda_enabled".as_ptr()) };
            let cuda_status_ptr =
                unsafe { GetProcAddress(handle, c"hex_last_search_used_cuda".as_ptr()) };
        ...
        fn set_cuda_enabled(&self, enabled: bool) -> Result<(), String> {
            let Some(set_cuda_enabled) = self.set_cuda_enabled else {
                return Err("DLL does not export hex_set_cuda_enabled".to_owned());
            };
            unsafe {
    ...
    pub(crate) fn main() {
        match parse_command(env::args().skip(1).collect()) {
            ...
            Err(err) => {
                eprintln!("{err}");
                eprintln!(
                    "usage:\n  hex_dll_harness bench --dll PATH [--case all|cuda-long|GROUP|NAME] [--budget N] [--threads N] [--repeat N] [--warmup N] [--cuda default|off|both] [--format pretty|tsv] [--color auto|always|never]\n  hex_dll_harness bench-compare --rust PATH --original PATH [--case all|cuda-long|GROUP|NAME] [--budget N] [--threads N] [--repeat N] [--warmup N] [--cuda default|off|both] [--min-ratio N] [--fail-on-mismatch true|false] [--format pretty|tsv] [--color auto|always|never]"
                );
                std::process::exit(2);
            },
        }
    ...
    fn bench(
        ...
        if settings.output.format == OutputFormat::Tsv {
            ...
            print_run_header("Brainstorm OVERHAULED DLL Benchmark", settings, cases.len());
        }
    ...
    fn bench_compare(
        ...
        if settings.output.format == OutputFormat::Tsv {
            ...
            print_run_header(
                "Brainstorm OVERHAULED DLL Benchmark: Rust vs Original",
                settings,
                cases.len(),
            );
        }
        ...
        for case in &cases {
            ...
            let (original_summary, original_skip) = if let Some(reason) = original_skip_reason(case)
            {
                ...
                match classify_legacy_probe(
                    case.seed_start.unwrap_or(""),
                    ...
                    LegacyProbe::OutOfBudget => (
                        None,
                        Some("legacy hit falls outside the requested search budget"),
                    ),
                    LegacyProbe::Hit { scanned } => {
                        let probe_result =
