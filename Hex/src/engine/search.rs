...
/// Clamp the public FFI budget to the finite seed space.
///
/// Non-positive budgets scan nothing; callers that want a search must provide
/// an explicit positive budget.
pub(crate) fn resolve_seed_budget(num_seeds: i64) -> i64 {
    num_seeds.clamp(0, SEED_SPACE)
}
...
fn search_filters(
    ...
    if num_seeds == 0 || cfg.shape == KernelShape::NoMatch {
        return None;
    }
