...
`BalatroSource/` is the literal game source and must not be committed to git.
...
- Vanilla run setup uses an all-caps seed input with a maximum length of eight.
  Its normal corpus is digits `1`-`9` plus `A`-`Z`; typed or pasted `0` is
  normalized to `O`. Generated seeds are exactly eight characters from the
  34-symbol corpus without `O` (`34^8 = 1,785,793,904,896` possibilities).
  Non-empty vanilla input has `35^1 + ... + 35^8 = 2,318,107,019,760`
  normalized strings. The UI also permits the empty string: `can_start_run`
  does not reject it, and Lua treats it as a present `args.seed`. Sources:
  `functions/button_callbacks.lua` (`toggle_seeded_run`, `text_input_key`,
  `paste_seed`, `can_start_run`, `start_setup_run`);
  `functions/UI_definitions.lua` (`G.UIDEF.run_setup_option`,
  `create_text_input`); `game.lua` (`Game:start_run`);
  `functions/misc_functions.lua` (`generate_starting_seed`, `random_string`).
...
- Boss defeat queues `ease_ante(1)` before refreshing the voucher and before
  `cash_out` opens the post-boss shop. That voucher and shop therefore use the
  next ante's pool and RNG keys; Small and Big tags are also refreshed under the
  incremented ante during `cash_out`. Sources: `functions/state_events.lua`
  (`end_round`); `functions/common_events.lua` (`ease_ante`);
...
- `cash_out` pays round dollars, resets shop/hand counters, sets
  ...
  `functions/button_callbacks.lua` (`G.FUNCS.cash_out`).
- `skip_blind` advances `G.GAME.blind_on_deck` and applies skip/tag effects
  without selecting or playing a round. A skipped Small or Big Blind therefore
  creates no shop and consumes no shop stock rolls. Source:
  `functions/button_callbacks.lua` (`G.FUNCS.skip_blind`).
...
- The Soul/Black Hole injection block runs only when there is no `forced_key`,
  `soulable` is true, and `c_soul` is not banned. Each special center also has
  a live-card duplicate gate that Showman bypasses.
  Tarot/Spectral/Tarot_Planet can become `c_soul`; Planet/Spectral can become
  `c_black_hole`; both use
  `pseudorandom('soul_'.._type..ante) > 0.997`. Source:
  `functions/common_events.lua` (`create_card`).
- A soulable Spectral card checks Soul first and Black Hole second, advancing
  the same `soul_Spectral<ante>` stream twice when both are eligible. A
  successful Black Hole check overwrites a Soul selected for that same card.
  Constructed pack cards set `used_jokers`, so later cards in that live pack
  skip a generated Soul or Black Hole unless Showman permits duplicates. When
  a pack ends, `G.FUNCS.end_consumeable` removes its UI and pack-owned
  `CardArea`; the resulting `Card:remove()` calls clear each gate if no copy
  remains in the Joker or consumable areas, so a later pack can roll the
  special again. Sources: `functions/common_events.lua` (`create_card`);
  `functions/button_callbacks.lua` (`G.FUNCS.end_consumeable`);
  `functions/UI_definitions.lua` (pack `CardArea`); `engine/ui.lua`
  (`UIBox:remove`, `UIElement:remove`);
  `card.lua` (`Card:set_ability`, `Card:remove`);
  `functions/misc_functions.lua` (`find_joker`); `cardarea.lua`
  (`CardArea:remove`).
- The `c_soul` ban is the outer injection guard. If it is banned, neither the
  Soul nor Black Hole special checks run. If only `c_black_hole` is banned,
  its check can still consume a roll and overwrite a same-card Soul, after
  which the banned forced key falls back to normal pool selection. Source:
  `functions/common_events.lua` (`create_card`).
...
- `reroll_shop` only removes/refills `G.shop_jokers`. It does not reroll the
  voucher or booster slots. Each ordinary, unforced refill calls
  `create_card_for_shop()` again, so it continues the same ante-keyed type,
  rarity, pool, sticker, and edition streams; there is no separate reroll
  suffix. Tutorial stock and `store_joker_create` tag replacements return
  before that normal generation path and do not advance those streams. Sources:
  `functions/button_callbacks.lua` (`G.FUNCS.reroll_shop`);
  `functions/UI_definitions.lua` (`create_card_for_shop`).
...
- Voucher selection uses `get_next_voucher_key(_from_tag)`, which samples the
  ...
  `'UNAVAILABLE'` entries. Normal selection uses `Voucher<ante>` because
  `get_current_pool()` appends the current ante; Voucher Tag replaces that key
  with the unnumbered `Voucher_fromtag`. Sources: `functions/common_events.lua`
...
- Leaving a shop removes its voucher `CardArea` and clears that area's cards.
  Merely seeing a voucher therefore does not exclude it from the next ante; an
  unredeemed voucher may be selected again. Redemption persists exclusion
  through `G.GAME.used_vouchers`. Sources: `functions/button_callbacks.lua`
  (`G.FUNCS.toggle_shop`); `engine/ui.lua` (`UIBox:remove`); `cardarea.lua`
  (`CardArea:remove`); `functions/common_events.lua` (`get_current_pool`).
- Redeeming a voucher sets `G.GAME.used_vouchers[center_key] = true`, clears
  ...
  `Card:apply_to_run`).
- Hieroglyph and Petroglyph each define `extra = 1` and immediately call
  `ease_ante(-center_table.extra)` when redeemed. Finishing the next Boss can
  therefore revisit an ante number whose
  `Voucher<ante>`, `Tag<ante>`, shop, and pack streams already advanced; keyed
  state continues rather than restarting. Sources: `game.lua` (voucher center
  definitions); `card.lua` (`Card:apply_to_run`);
  `functions/misc_functions.lua` (`pseudoseed`);
  `functions/state_events.lua` (`end_round`); `functions/button_callbacks.lua`
  (`G.FUNCS.cash_out`).
...
- `get_pack(_key, _type)` first forces the first shop pack to a normal Buffoon
  ...
  banned; this uses unseeded `math.random(1, 2)` for `_1`/`_2` and does not
  advance `_key..ante`. Otherwise it weights `G.P_CENTER_POOLS.Booster` by
  `weight`, optional `kind`, and `banned_keys`, then polls with
  `pseudorandom(pseudoseed(key..ante))`. Source: `functions/common_events.lua`
  (`get_pack`).
...
- On the first ordinary traversal of each ante in an unbanned, no-skip run,
  ante 1 has shops after Small and Big: four physical booster slots but only
  three `shop_pack1` advances because the forced Buffoon occupies the first
  slot without a seeded pack roll. Ante 2 and later have an entry shop after
  the previous boss plus Small and Big shops, for six `shop_pack<ante>`
  advances. Ante reduction can revisit a numeric ante and advance that same
  stream further. Skipping Small or Big forgoes that blind's shop, and if no
  earlier shop occurred the forced Buffoon moves to the first shop that
  actually opens. Sources: `functions/state_events.lua`
  (`new_round`, `end_round`); `functions/button_callbacks.lua`
  (`G.FUNCS.skip_blind`, `G.FUNCS.cash_out`); `game.lua`
  (`Game:update_shop`); `card.lua` (`Card:apply_to_run`);
...
- Each Arcana card polls the global, unnumbered `omen_globe` stream only while
  Omen Globe is active, choosing Spectral or Tarot creation. If the card is
  soulable, has no forced key, and the ban/duplicate gates permit it,
  `create_card()` conditionally polls `soul_Spectral<ante>` or
  `soul_Tarot<ante>`; Spectral creation can poll its Soul stream twice for Soul
  then Black Hole. An allowed special skips normal pool selection. Otherwise,
  the selected type advances `Spectralar2<ante>` or `Tarotar1<ante>`. Because
  pack contents are generated on open, redeeming Omen Globe before opening a
  pack in the same shop affects that pack immediately. Sources: `card.lua`
  (`Card:open`, `Card:redeem`, `Card:apply_to_run`);
  `functions/common_events.lua` (`create_card`).
...
- Later Souls continue the same global `Joker4` stream. A Legendary created by
  an earlier Soul remains excluded through the index-preserving Joker pool
  while it is owned, so selection may use `Joker4_resampleN`; selling/removing
  it clears that exclusion when no copy remains. Showman bypasses the duplicate
  exclusion. Sources: `functions/common_events.lua` (`get_current_pool`,
  `create_card`); `card.lua` (`Card:set_ability`, `Card:remove`).
...
- Tag pool culling preserves positions. A tag becomes `'UNAVAILABLE'` when its
  required center is not discovered, its `min_ante` is above the current ante,
  or its key is banned. If an `'UNAVAILABLE'` placeholder is selected,
  selection resamples with `Tag<ante>_resampleN`. For example, Negative Tag has
  `min_ante = 2` and requires discovery of the Negative edition center. Sources:
  `game.lua` (`P_TAGS`);
  `functions/common_events.lua` (`get_current_pool`, `get_next_tag_key`).
...
- Call sites are distributed: `add_tag()` triggers `tag_add`; shop creation
  ...
  `functions/state_events.lua`; `functions/button_callbacks.lua`.
- Uncommon and Rare Tags use `store_joker_create` and can replace a normal shop
  stock roll with a forced-rarity Joker. Charm Tag uses `new_blind_choice` and
  opens a Mega Arcana pack instead; it does not force a shop Joker. Sources:
  `game.lua` (`P_TAGS`); `tag.lua` (`Tag:apply_to_run`);
  `functions/UI_definitions.lua` (`create_card_for_shop`).
...
- Taking a Charm or Ethereal Tag both forgoes the skipped blind's shop and,
  through `new_blind_choice`, immediately opens a free Mega Arcana or normal
  Spectral pack before the next blind is played. Its contents use the current
  ante's ordinary Tarot/Spectral Soul and pool streams, so tag rewards and shop
  packs must be simulated in physical event order rather than as independent
  routes. Sources: `functions/button_callbacks.lua`
  (`G.FUNCS.skip_blind`); `tag.lua` (`Tag:apply_to_run`); `card.lua`
  (`Card:open`); `functions/common_events.lua` (`create_card`).
...
- `pseudoseed(key, predict_seed)` drives deterministic RNG:
  ...
  - Both transformed paths round the transformed state through
    `tonumber(string.format('%.13f', ...))` before averaging. Exact replicas
    must preserve this decimal round-trip.
...
- Pools intentionally keep `'UNAVAILABLE'` placeholders before sampling, then
  resample with `<pool_key>_resampleN` if selected. The retry counter begins at
  one and increments before sampling, so the first retry is `_resample2`.
  Native search tables must not compact unavailable entries unless they
  reproduce the same resampling distribution. Sources:
  `functions/common_events.lua` (`get_current_pool`, `get_next_voucher_key`,
  `get_next_tag_key`, `create_card`).
...
- Normal voucher and tag selection use `Voucher<ante>` and `Tag<ante>`.
  Small then Big consume consecutive advances of the same `Tag<ante>` stream
  at run start and after each boss. Source: `game.lua` (`Game:start_run`);
  `functions/button_callbacks.lua` (`G.FUNCS.cash_out`);
  `functions/common_events.lua` (`get_current_pool`, `get_next_voucher_key`,
  `get_next_tag_key`).
...
- Shop and Buffoon Jokers generated through `create_card()` advance
  `etperpoll<ante>` or `packetper<ante>` before stake modifiers decide whether
  that value produces an Eternal/Perishable sticker. Rental keys
  `ssjr<ante>`/`packssjr<ante>` are read only when rentals are enabled. These
  keys are independent from Joker type, rarity, pool, and edition streams, so
  stake sticker rules do not shift those other results. Tutorial-forced shop
  cards construct `Card(...)` directly and bypass this sticker RNG. Sources:
  `game.lua` (`Game:start_run` stake modifiers);
  `functions/UI_definitions.lua` (`create_card_for_shop`);
  `functions/common_events.lua` (`create_card`).
...
- The Rust native search is the implementation for Brainstorm OVERHAULED's
...
- Native `SEED_SPACE` is the empty seed plus every length-one-through-eight
  string over vanilla's normalized 35-symbol input corpus
  (`2,318,107,019,761` states), not only the `34^8` naturally generated seeds.
  Source: current Rust implementation (`Hex/src/seed.rs`).
...
- Observatory does not alter pack generation. The voucher's gameplay effect is
  checked when a held Planet card matches the scoring hand. In Hex,
  `observatory` is a search shortcut for "ante-1 Telescope voucher plus
...
- Perkeo's own Joker effect later copies a random consumable using the `perkeo`
  RNG key. Hex's Perkeo search only needs the Soul-to-legendary path that
